...
7. Теперь перейдём к написанию логики пользовательского интерфейса:
    ...
    4. Сначала реализуем логику выходя из игры. У кого-то могли возникнуть вопросы, что такое `#if UNITY_EDITOR`, `#else`, `#endif` это “Условная компиляция”. В данном случае было сделано условие, если код запускается в редакторе, то происходит остановка его с помощью его функций, если же код запущен не в редакторе, то мы производим выход из нашей игры;
        ...
        **Разбор**
        ...
        - **`#if UNITY_EDITOR`**: Если код компилируется в среде Editor, включается блок кода ниже этой директивы. В данном случаеесли игра запущена в редакторе, выполнение остановится и выйдет из режима проигрывания (`isPlaying = false`).
        - **`#else`**: Если условие `#if UNITY_EDITOR` не выполняется (есть компиляция идет не для редактора, а для других платформвыполняется альтернативный блок кода. В данном случае метод `Application.Quit()`, который завершает приложение.
        - **`#endif`**: Эта директива завершает условный блок компиляцииуказывая компилятору конец условного кода.

        **Зачем это нужно?**

        - **Платформенная независимость**: Позволяет кроссплатформенные приложения, управляя различными аспектами кода разных платформ (например, Windows, iOS, Android).
        - **Оптимизация**: Исключает ненужный код для конфигурации, уменьшая размер и потенциальные ошибки.
        - **Удобство разработки**: Упрощает тестирование и отладку, так можно писать и тестировать код, который выполняется только в разработки, не влияя на конечную сборку.
        ...
        **Примеры условий**
        ...
        - Можно использовать и пользовательские символы, добавляемые настройках проекта.

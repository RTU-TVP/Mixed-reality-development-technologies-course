...
!<https://docs.unity3d.com/Packages/com.unity.xr.meta-openxr@2.0/manual/images/scene-capture.png>
...
1. Сделаем свою отрисовку поверхностей:
    ...
    5. Напишем скрипт **ARPlaneColorizer** для отработки логики отрисовки поверхностей:
        ...
        - **Разъяснение к коду:**
            ...
            `ARBoundingBoxColorizer` — это компонент Unity, который визуализацией и цветом границ (bounding boxes) объектовобнаруженных с помощью дополненной реальности (AR). Он для автоматического назначения цвета границам на основе классификации и может переключать их видимость. Компонент AR Foundation и ARBoundingBox для работы с AR-объектами.

            ### Компоненты

            - **ARBoundingBox**: Определяет границы объекта, обнаруженного помощью AR.
            - **MeshRenderer**: Отвечает за визуализацию границ, доступ к материалу и его свойствам, таким как цвет и видимость.

            ### Поля

            - **defaultColorAlpha**: Прозрачность цвета по умолчанию для (можно настроить в Unity Inspector).
            - **_defaultColor**: Цвет, назначаемый границам на основе классификации.
            - **_boundingBox**: Ссылка на компонент ARBoundingBox, для получения размеров и классификации границ.
            - **_meshRenderer**: Ссылка на компонент MeshRenderer, для управления визуализацией и цветом границ.

            ### Свойства

            - **isVisualise**: Управляет видимостью границ. Если `true`, отображаются; если `false`, они скрыты. Состояние синхронизируется с полем `enabled` компонента `MeshRenderer`.

            ### Методы

            - **Awake()**: Метод инициализации, который вызывается при объекта. Он находит компоненты `ARBoundingBox` и `MeshRendererустанавливает цвет и прозрачность границ на основе их и задает их начальное состояние визуализации.
            - **GetColorByClassification(classification)**: Статический метод, который возвращает цветсоответствующий классификации границ. Если классификация неизвестнавозвращается серый цвет.

            ### Логика работы

            1. **Инициализация**:

                - В методе `Awake()` компонент находит `ARBoundingBox` `MeshRenderer` среди дочерних объектов.
                - Если один из компонентов не найден, выводится сообщение ошибке.
                - Устанавливается цвет границ на основе их классификации настраивается прозрачность.
                - Размеры границ устанавливаются в соответствии с размерамизаданными в `ARBoundingBox`.
                - Состояние визуализации устанавливается в соответствии значением свойства `isVisualise`.
...
2. Сделаем свою отрисовку ограничительной рамки:
    ...
    5. Напишем скрипт **ARBoundingBoxColored** для отработки логики ограничительной рамки:
        ...
        `ARBoundingBoxColorizer` — это компонент Unity, предназначенный управления визуализацией и цветом границ (bounding boxes) объектовобнаруженных с помощью технологий дополненной реальности (ARКомпонент работает в связке с `ARBoundingBox` и назначает цвет границам на основе их классификации. Это полезно визуального разделения и отладки AR-объектов.

        ### Основные функции

        - **Управление видимостью границ**: Позволяет включать или визуализацию границ объектов.
        - **Цветовая индикация**: Назначает цвета границам объектов основе их классификации, что позволяет легко различать разные объектов.

        ### Компоненты

        - **ARBoundingBox**: Определяет границы объекта в AR, информацию о размере и классификации объекта.
        - **MeshRenderer**: Отвечает за визуализацию границ и материалами, включая их цвет и прозрачность.

        ### Поля

        - **defaultColorAlpha**: Прозрачность цвета по умолчаниюустанавливаемая через Unity Inspector. Значение по умолчанию — 0.25.
        - **_defaultColor**: Цвет, определяемый на основе границ.
        - **_boundingBox**: Ссылка на компонент `ARBoundingBox`, содержит информацию о границах объекта.
        - **_meshRenderer**: Ссылка на компонент `MeshRenderer`, за отображение границ объекта.
...
4. При разработке могут возникать различные ошибки, часть которых можно отследить в редакторе в консоли, но бывают ситуации, когда они возникают только во время тестирования конечной сборки на устройстве и там нет консоли для отладки. Давайте добавим её и так же сделаем включение и выключения отображения:
    1. Сначала импортируем готовый ресурс с консолью для XR продуктов - <https://github.com/platinio/UnityVRDebug>;
...
В этом курсе вы узнали, как создавать приложения дополненной реальности для устройств Oculus Quest с использованием Unity. Вы изучили основные концепции и инструменты, необходимые для создания приложений дополненной реальности, и научились применять их на практике.
